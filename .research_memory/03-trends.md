...
## 2026-07-17：动态 RAG 正从“内部不确定性阈值”转向“充分性、效用与证据管理”

- ETC/DRAGIN 代表的 entropy/attention 触发仍是重要底座，但 QuCo-RAG 等工作已直接指出内部信号存在失校准与自信错误问题。
- 2025-2026 近邻工作同时向三个方向集中：问题/证据充分性与知识缺口（S2G-RAG）、检索后质量和噪声处理（CRAG、D²-RAG）、对生成任务实际贡献的 utility/evidence valuation（CUE-R 等）。
- 因此仅做“自适应阈值”“gap query”“噪声过滤”“轻量收益模型”都已拥挤。当前可争取的组合空间是：生成中间状态上的 skip/多查询配对终局收益、跨时间查询收益曲线和选择后风险校准。
- 成本仍是部分论文的动机，但是否适用于当前本地 BM25/短上下文设置必须实测，不能直接移植为论文主叙事。
