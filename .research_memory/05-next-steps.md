...
# 2026-07-18 ETC 独立密集时机诊断进展

- [x] 新增 `dense_timing_v1`：在答案标记出现前采样全部句界、每 12 个生成 token 的网格、首次 ETC 触发和答案前锚点；ETC 只作为候选之一，不再决定候选资格。
- [x] 每题最多 8 个状态；超预算时保留首次 ETC/答案前锚点，其余沿时间轴分层选取。旧三检查点配置保持不变。
...
# 2026-07-18 ETC 下一阶段（归因审计后）

- [x] 完成 20 qid、256 个动作的查询—证据—桥接归因，最终口径 `query_evidence_attribution_v3`。
- [x] 停止扩大“密集检索时机”实验；现有证据只支持 ETC 时机作为强基线，不支持把新时机学习作为论文主贡献。
