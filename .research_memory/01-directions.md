...
## 🆕 2026-07-17：当前主线——ETC 上的校准反事实检索收益选择

> 本节覆盖下方 ReasonRAG、LogicRAG、DocRE 等历史主线判断。详细审计、复现结果、相关工作边界与公式见 `08-etc-cura.md`。

- **底座**：ETC（动态及时检索 RAG）。论文方法与开源代码确实不同；后续必须区分 `ETC-paper-spec`、`ETC-release` 和用户的高效在线等价执行版本。
- **当前方法候选**：CURA（暂定名），但已从“成本加权净效用”修订为**校准的反事实检索收益动作选择**。
- **核心对象**：在同一生成状态下，比较 `skip` 与多个候选查询检索后继续生成的最终答案差值：
  \[
  B_t(q)=S(Y_t^q,y^*)-S(Y_t^{skip},y^*).
  \]
- **何时检索**：某个候选查询的校准收益下界首次为正；**查什么**：选择收益下界最高的查询。
- **核心动机**：不是节省廉价检索调用，而是识别“查询就绪窗口”，避免无效、过早、含糊或污染性证据干预。
- **新颖性边界**：不能声称首次做动态检索、知识缺口、噪声过滤、联合 when/what 或证据效用；必须落在“生成中间状态的 skip/多查询配对终局收益、跨时间收益曲线、选择后风险校准”组合上。
- **保守备选**：Cal-ETC，仅用真实配对收益监督轻量触发器，查询仍保持 ETC/DRAGIN QFS。
- **强制反证基线**：`Always-Retrieve + Post-Retrieval Filter`。若其答案质量、污染率和实际延迟与 CURA 相当，则当前设置下“何时发起检索”没有足够研究价值，应转向“何时使用证据”。

---

## 🆕 2026-06-10：新主候选 —— agent 自演进（研究内容二 / PRM）

> **导师建议弃 DocRE/EE，改做航天科工指控系统 agent 项目**（本子在 repo 根目录）。这是当前**主候选**，DocRE/TTM-RE 全套方案降级为**兜底**。决策链见 `04-decisions.md`(2026-06-10)。

- **方向**：可信 agent 自演进。本子三研究内容（业务图自构建 / 步骤级反馈归因 / 约束感知导航）均落在 2026 热点上。
- **用户倾向**：研究内容二 = **步骤级错误归因 / 过程奖励模型（PRM）**。理由：最不水（LLM-judge 在 Who&When 仅 ~14%，硬问题有背书）、最贴 B 会"训模型"经验、有公开 benchmark 不依赖指控数据。
- **论文两章**：Ch1 步骤级归因（训 verifier/PRM）→ Ch2 反馈驱动知识提炼/闭环（接"自演进"）。
- **可行性 gate**：① 导师是否要求用项目真实数据；② GPU 可用性。两点待用户问导师。
- **兜底方向**：研究内容一（业务图要素抽取 = 文档级 NER+RE，最贴老本行，学术亮度低一档）；再不行回 DocRE/TTM-RE（见下方旧方案）。
...
## 2026-07-07 更新：依据训练证据重置方向

- 决策：停止将 RL 作为论文主线。
- 原因：本地最新结果显示 `DPO+LoRA` 相比全量 DPO 已有约 `-3` 个点差距；在当前实验室算力下，RL 路线成本更高、迭代更慢。
- 新主线：非 RL 的 Agentic RAG。
- 优先改造点（非 RL）：
  - 过程质量的奖励构造（规则优先，可选轻量模型校正）
  - 仅作为推理或数据构造策略的 search / rollout 改造（不做策略梯度更新）
  - 训练目标限定在可行集合：`LoRA-DPO` 或 `全量 SimPO`
- 基线策略：
  - 优先 2026 年开源基线（必须有代码，优先有权重）
  - 回避依赖重型在线 RL 循环的方法
