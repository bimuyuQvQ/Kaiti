...
### 运行安全红线（2026-06-25 新增）

- **严禁再次触发系统 OOM**：任何高内存任务（索引构建/大规模推理/训练）在运行前必须先做内存风险评估。
...
- 本地实验新增结论：
  - `DPO 全量微调` 对比 `DPO + LoRA`，`DPO + LoRA` 准确率约低 `3` 个百分点。
- 算力约束升级为硬约束：
  - 可行训练仅包括：`DPO+LoRA` 或 `全量 SimPO`
  - 不再可行的主线：需要高成本 rollout 与策略更新的 RL 训练流程
- 论文优先级：
  - 不以复现高算力 RL SOTA 为目标
  - 以“有限算力下可复现、可论证的非 RL Agentic RAG 路线”为目标

## 2026-07-10 更新（项目协作规则）

- 除非用户明确要求修改，否则涉及算法核心代码时优先做增量更新：新增脚本、新增配置、新增可选开关，尽量不要改动已有老代码路径。
- 用户与 Codex 交互的 Windows 机器没有显卡；GPU 任务需要先把代码提交/推送到仓库，再在实验室 Linux 服务器上拉取运行。

## 2026-07-17 更新（当前主线与最新约束，覆盖旧方向判断）

- 当前毕业论文主线已转为：**以 ETC 为基线的动态及时检索 RAG 改进**。旧记忆中“排除 RAG 学术路线”“ReasonRAG 是当前主线”等判断已过时，仅保留历史追溯价值。
- 目标不是对 ETC 做零碎调参，而是在保留“根据生成状态决定何时检索”的大方向下，形成具有统一动机、可复现算法和可验证贡献的方法。
- 用户认可“查什么”应与“什么时候查”联合考虑，但否定把检索调用、延迟和上下文成本预设为核心动机：当前本地 BM25/Elasticsearch 调用可能很便宜，上下文开销是否显著必须实测。
- 当前更可信的动机是：**外部证据是状态依赖的信息干预，可能改善、无效或破坏生成；及时检索应发生在知识缺口已经足够明确、查询已经就绪且证据负效应风险可控的生成位置。**
- 成本、延迟、上下文 token 继续报告，但降为次要系统指标；核心指标改为最终答案收益、负收益检索率、正确→错误翻转、查询证据覆盖和校准风险。
- 后续实现继续遵守增量原则：原始 ETC 代码、原配置和原结果不原地修改；新增研究入口、模块、配置和输出目录。用户审核方案前不开始算法编码或耗时实验。
