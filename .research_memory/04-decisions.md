...
### 2026-07-17：ETC/CURA 成为当前主线，放弃以检索成本作为核心动机

- **背景**：完成 ETC 论文、开源代码、用户修复版本、评测协议和近邻工作的研究审计。用户进一步指出，本地 BM25/Elasticsearch 调用和延迟很低，上下文成本也没有现成证据证明在当前设置中显著。
- **选项**：A. 继续用“答案收益－调用/延迟/上下文成本”的净效用；B. 以噪声过滤为主；C. 以状态依赖的检索收益、查询就绪窗口和有害证据风险为主。
- **决定**：选择 C。保留 CURA 暂定名，但核心标签改为 `B_t(q)=Score(retrieve q)-Score(skip)`；调用、延迟和 token 只作为报告指标或后续约束，不进入第一版核心目标。
- **理由**：
  1. 检索成本是否重要应由当前系统实测决定，不能作为预设前提；
  2. “噪声必然有害”同样过度简化，相关工作表明噪声可能有正、零、负效应；
  3. 更统一的对象是“特定生成状态下，特定查询带来的终局答案效应”；
  4. 该对象自然统一何时检索、查询选择、污染和校准风险；
  5. 强制加入 `Always-Retrieve + Filter` 作为反证，允许实验否定 pre-retrieval timing 的必要性。
- **实施约束**：用户审核实现方案前，不修改 ETC 算法代码、不启动耗时实验；后续采用增量目录和配置切换，保留所有旧代码和 baseline。

### 2026-07-17：ETC 复现必须同时报告原始抽取与修正抽取

- **背景**：HotpotQA alpha=1.0 在线并行运行 1000 条已完成。原始仓库抽取为 `EM=0.3050 / F1=0.4268`；修正重复 `the answer is` 后为 `EM=0.3740 / F1=0.4924`；论文为 `EM=0.272 / F1=0.487`。
- **决定**：复现主表同时报告两种抽取，前者表示严格代码复现，后者表示评测污染修正；不得只选择更接近论文的一项。后续所有方法比较统一使用版本化的修正抽取器，同时在附录报告原始口径和重复答案率。
- **依据**：1000 条中 231 条最终答案变化，169 条包含至少两个 `the answer is`；修正版保留原始 305 条 EM 并新增 69 条，无正确样本被改错。新增 EM 中 47 条、F1 提升中 99 条直接来自重复答案模板。
