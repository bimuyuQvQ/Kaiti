...
## ⚠️ 2026-07-17 当前主线（最高优先级，覆盖下方历史方向）

- 当前底座是 **ETC 动态及时检索 RAG**，不是 ReasonRAG、LogicRAG 或 DocRE。详细入口见 [`08-etc-cura.md`](./08-etc-cura.md)。
- 研究主线暂定为 CURA：在生成中间状态上，通过 `skip` 与多个查询的离线配对 rollout 学习查询特异的最终答案收益，并用一侧校准下界决定何时、用什么查询检索。
- 已放弃把“检索调用、延迟、上下文成本”作为核心动机；当前主线是查询就绪窗口与外部证据的正/零/负状态依赖效应。
- 强制加入 `Always-Retrieve + Post-Retrieval Filter` 作为反证。若它与 CURA 相当，则应承认当前场景不需要 pre-retrieval timing。
- 原 ETC 文件和已有结果必须保持不动；新代码采用增量入口、模块、配置和输出目录。当前处于方案审核阶段，用户确认前不开始算法编码或耗时实验。
- HotpotQA alpha=1.0 的 1000 条结果：原始抽取 `0.3050/0.4268`，修正抽取 `0.3740/0.4924`，论文 `0.272/0.487`；复现报告必须同时列两种口径。

## ⚠️ 2026-06-10 重大转向（先读这条，覆盖下方第 3 节）

导师建议**弃 DocRE/EE**，改做**航天科工指控系统 agent 自演进项目**（本子在 repo 根目录 `项目立项申报书V1.0...doc`）。当前**主候选 = 研究内容二：步骤级反馈归因 / 过程奖励模型 PRM**，scope 为"自己找题做毕业论文"。下方第 3 节"已锁定 DocRE"**已降级为兜底**。完整决策链见 `04-decisions.md`(2026-06-10) + `01-directions.md` 顶部 + `05-next-steps.md` 顶部。

## 3. 之前锁定、现降级为兜底的方向（DocRE，2026-06-10 前）
...
- 代码协作新增规则（2026-07-10）：除非用户明确要求，算法核心代码优先做增量更新，不直接改老逻辑；Windows 交互机无显卡，GPU 实验要通过 git 同步到 Linux 服务器运行。
